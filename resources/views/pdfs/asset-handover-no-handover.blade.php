...
<head>
    <meta charset="utf-8">
    <title>Acta de Entrega — {{ $asset->hostname ?? $asset->asset_tag ?? "Activo #{$asset->id}" }}</title>
    <style>
        * { box-sizing: border-box; }
        ...
        .header-table td { border: 1px solid #000; padding: 2px 5px; vertical-align: middle; }
        .logo-cell { width: 22%; text-align: center; }
        .logo-cell img { max-height: 32px; max-width: 85px; }
        .title-cell { font-weight: bold; text-align: center; font-size: 9.5pt; }
        .meta-cell { width: 28%; font-size: 7pt; }
        .meta-cell div { line-height: 1.2; }

        .meta-table { width: 100%; border-collapse: collapse; margin-top: 0; }
        .meta-table td { border: 1px solid #000; padding: 1.5px 5px; }
        .meta-table .label { width: 18%; font-weight: bold; font-size: 7.5pt; }
        .meta-table .value { font-size: 7.5pt; }
        ...
        .equipment-title { font-weight: bold; font-size: 7.5pt; margin-bottom: 2px; }
        ...
        .sig-header { text-align: center; font-weight: bold; height: 14px; padding: 1px 5px; }
        .sig-line { height: 30px; text-align: center; padding-top: 18px; font-style: italic; color: #555; }
        .sig-accepted {
            height: 30px;
            text-align: center;
            padding: 3px 5px;
...
<table class="meta-table">
    <tr>
        ...
        <td class="value">{{ \Carbon\Carbon::parse($acceptedAt)->format('d/m/Y') }}</td>
    </tr>
    <tr>
        <td class="label">REFERENCIA</td>
        <td class="value">Entrega de {{ strtoupper($asset->type ?? 'EQUIPO') }}</td>
    </tr>
    <tr>
        <td class="label">RECIBE</td>
        <td class="value">{{ strtoupper($asset->user?->name ?? '—') }}</td>
    </tr>
    <tr>
        <td class="label">CARGO</td>
        <td class="value">{{ strtoupper($asset->user?->position ?? '—') }}</td>
    </tr>
    <tr>
        <td class="label">UBICACIÓN</td>
        <td class="value">{{ strtoupper($asset->field ?? $asset->user?->field ?? '—') }}</td>
    </tr>
</table>
...
<div class="equipment-block">
    ...
    <table class="equipment-grid">
        <tr>
            ...
            <td class="field-value">{{ $asset->asset_tag ?? '—' }}</td>
            <td class="field-label">Tipo Activo:</td>
            <td class="field-value">{{ strtoupper($asset->type ?? '—') }}</td>
        </tr>
        <tr>
            <td class="field-label">Fabricante:</td>
            <td class="field-value">{{ strtoupper($asset->manufacturer ?? '—') }}</td>
            <td class="field-label">Modelo:</td>
            <td class="field-value">{{ strtoupper($asset->model ?? '—') }}</td>
        </tr>
        <tr>
            <td class="field-label">Serial:</td>
            <td class="field-value">{{ $asset->serial_number ?? '—' }}</td>
            <td class="field-label">Hostname:</td>
            <td class="field-value">{{ $asset->hostname ?? '—' }}</td>
        </tr>
        <tr>
            <td class="field-label">Condición:</td>
            <td class="field-value">{{ strtoupper($asset->condition ?? 'BUENO') }}</td>
            <td class="field-label">Campo:</td>
            <td class="field-value">{{ strtoupper($asset->field ?? '—') }}</td>
        </tr>
        <tr>
            ...
            <td class="field-value" colspan="3">
                ...
                @if ($asset->project?->name)
                    &nbsp;&nbsp;<strong>{{ strtoupper($asset->project->name) }}</strong>
                @endif
            </td>
        </tr>
    </table>
...
<table class="signatures-table">
    ...
    <tr>
        <td class="sig-row"><strong>N° Identidad:</strong></td>
        <td class="sig-row"><strong>N° Identidad:</strong> {{ $asset->user?->identification ?? '' }}</td>
    </tr>
    <tr>
        <td class="sig-row"><strong>Nombre:</strong></td>
        <td class="sig-row"><strong>Nombre:</strong> {{ strtoupper($asset->user?->name ?? '') }}</td>
    </tr>
</table>
